...
interface UploadStepProps {
  noteName: string;
  selectedOption: "Document" | "Image" | "Text" | null;
  onNoteCreated: (noteId: string) => void;
}

const UploadStep: React.FC<UploadStepProps> = ({ noteName, selectedOption, onNoteCreated }) => {
  ...
  const handleUpload = async () => {
    ...
    if (selectedOption === "Document") {
      ...
      alert(`File ${selectedFile.name} processed successfully!`);
    }
    // Handle Image option
      ...
      try {
        ...
        alert(`Image ${selectedFile.name} processed successfully!`);
      } catch (error) {
    ...
    // Notify the parent component that the note was created
    const newNoteId = data?.[0]?.id;
    if (newNoteId) {
      onNoteCreated(newNoteId);
      // Optionally, you can navigate here if the modal is closed afterwards
      // navigate(`/note/${newNoteId}`);
    }
  };

  if (!selectedOption) {
    return null; // Or render a message like "Please select an option first"
  }
  ...
  return (
    <div className="p-6 max-w-md mx-auto">
      ...
      {selectedOption === "Text" && (
        <>
          <h1 className="text-2xl font-bold text-center mb-4">Enter your text</h1>
          <textarea
            value={textContent}
